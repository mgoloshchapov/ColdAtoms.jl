...
const Id1 = g1 ⊗ dagger(g1) .+ p1 ⊗ dagger(p1) .+ r1 ⊗ dagger(r1) .+ gt1 ⊗ dagger(gt1) .+
            zero1 ⊗ dagger(zero1)
    ...
    for i in 1:N
        ...
        Ht = TimeDependentSum(
            [
                ...
                t -> Ω_blue(
                    dist + X2(t) - X0, Y2(t) - Y0, Z2(t), blue_laser_params_second) /
                     2.0,
                t -> conj(
                    Ω_blue(dist + X2(t) - X0, Y2(t) - Y0, Z2(t), blue_laser_params_second) /
                    2.0,
                ),
                t -> V_rr
            ],
...
function direct_CZ_simulation(
    ...
    #ρ0 = ψ0 ⊗ dagger(ψ0); #ψ0
    ρ0 = copy(ρ_1)
    ρ_temp1 = [zero(ρ_1) for _ in 1:length(tspan1)];
    ρ_temp2 = [zero(ρ_1) for _ in 1:length(tspan2)];
    ρ_mean = [zero(ρ_1) for _ in 1:(length(tspan1) + length(tspan2))];
    ρ_temp = [zero(ρ_1) for _ in 1:(length(tspan1) + length(tspan2))];
    ρ2_mean = [zero(ρ_1) for _ in 1:(length(tspan1) + length(tspan2))];
    ...
    for i in 1:N
        ...
        Ht_0 = TimeDependentSum(
            ...
            direct_operators            #[zz1, zz2, nr1, nr2, σgr1, σrg1,  σgr2, σrg2, rr]
        );
        ...
        Ht_phase = TimeDependentSum(
            [
                ...
                t -> Δ01,
                t -> Δ02,
                t -> exp(1.0im * phase) * Ω(X1(t), Y1(t), Z1(t), blue_laser_params_first) /
                     2.0,
                t -> conj(
                    exp(1.0im * phase) * Ω(X1(t), Y1(t), Z1(t), blue_laser_params_first) /
                    2.0,
                ),
                t -> exp(1.0im * phase) * Ω(X2(t), Y2(t), Z2(t), blue_laser_params_second) /
                     2.0,
                t -> conj(
                    exp(1.0im * phase) * Ω(X2(t), Y2(t), Z2(t), blue_laser_params_second) /
                    2.0,
                ),
                t -> V_rr
            ],
